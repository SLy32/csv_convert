...
variants = requests.get('http://pheweb.sph.umich.edu/api/manhattan/pheno/600.json').json()['unbinned_variants']
...
for x in itertools.product(*[(True, False)]*4):
    ...
    name = 'local-fields-' + '-'.join(k for k in sorted(args) if args[k])
    make_pheno(name, *x)
